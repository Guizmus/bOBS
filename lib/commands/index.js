...
var tchat_observers = []
async function initialize (_APIs,_config) {
    APIs = _APIs
    config = _config
    whitelist = config.commands.whitelist
    const command_files = fs.readdirSync("./user_commands");
    command_files.forEach(function (file) {
        if (file.match("[^\\.]+\.commands\.js")) {
            var new_command_file = require(process.cwd()+"/user_commands/"+file)
    ...
    command_names.forEach(function(command_name){
        var new_command = new command_list[command_name]()
...
function trigger(trigger,params) {
    if (!trigger.type) {
        trigger = {
            type : "direct call",
            name : trigger
    ...
    switch(trigger.type) {
        case "direct call" : 
            if (command_list[trigger.name].triggers["direct call"])
                return command_list[trigger.name].trigger(trigger,params)
            break;
        case "tchat" :
            ...
            if (Object.keys(tchat_commands).includes(trigger.alias))
                return command_list[tchat_commands[trigger.alias]].trigger(trigger,params)
            break;
        case "channel points" :
...
exports.USERLEVEL_MODERATOR = USERLEVEL_MODERATOR
const USERLEVEL_WHITELIST = 4
exports.USERLEVEL_WHITELIST = USERLEVEL_WHITELIST
const USERLEVEL_ADMIN = 5
