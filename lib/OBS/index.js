...
function play_sound(sound_name,duration) {
    if (soundboard_items[sound_name].currently_enabled) {
        call("SetSceneItemEnabled",{"sceneName":"Soundboard","sceneItemId":soundboard_items[sound_name].sceneItemId,"sceneItemEnabled":false}).then(function(d){
            soundboard_items[sound_name].currently_enabled = false;
            setTimeout(function(){
                play_sound(sound_name,duration)
            },1)
        })
        return false;
    }
