...
class WebModule {
    ...
    get_url(get_params=false,file="index.html") {
        var url = "http://localhost:"+config.WebModules.http_port + "/" + this.path + "/display/" + file
        if (get_params && get_params.length) {
            url = url + "?" + (new URLSearchParams(get_params)).toString()
        }
        return url;
    }
    async call(urlObj,req,res,data) {
        const url_parts = urlObj.pathname.split("/").slice(1)
        const key = url_parts[1];
        if(!this.#calls[key]) {
            return false;
        }
        const result = await this.#calls[key]({
        ...
        try {
            ...
            console.log("Error processing webmodule call",key,result,err)
        }
        return true;
    }
    #calls = {}
...
function WebModule_listener (req,res) {
    ...
    if (!!used_module) {
        switch (url_parts[1]) {
            case "display" :
                const base_path = used_module.name == "BaseModule" ? __dirname+'/' : process.cwd()+"/user_modules/"
                const file_path = base_path+used_module.name+"/"+(url_parts.slice(2).join("/"))
            ...
            default :
                ...
                req.on('end', async () => {
                    ...
                    await used_module.call(urlObj,req,res,JSON.parse(data))
                    res.end();
                })
                break;
        }
...
function unload_file(file) {
    delete(files_loaded[file])
}
exports.unload_file = unload_file;
